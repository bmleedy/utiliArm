...
// 60r/1min * 1min/60s * 200 counts/1r = 200 counts/s =
//        0.005 s/count = 5 msec/ct = 5000 usec/ct
...
#define STEPPER_STEPS_PER_DEGREE  20    // measured = 1750 steps per 90 deg
                                        // = 19.4444...

StepperAxis::StepperAxis(int16_t default_position,  // default position
// potentiometer(analog) or limit switch digital for analog_position = false VV
            uint8_t sensor_pin,
            ...
            int16_t zero_counts,   // what counts equals zero position
  ...
  if (analog_position) {
    ...
    // sense position based on limit switch and steps
...
int16_t StepperAxis::read_position() {
  ...
  this->current_position = (raw_counts - this->zero_counts) * 10
                           / counts_per_degree;
...
// Note, to make the motor move at 60rpm, this must
// be called at least every 2.5ms (400Hz)
void StepperAxis::move_constant_velocity(int8_t direction) {
  ...
  if (delta_us > STEPPER_MICROS_PER_HALFSTEP) {
    ...
    this->step_counts += direction;  // add or subtract one
                                     // based on direction I'm moving
  }
}
