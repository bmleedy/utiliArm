...
#define STEPPER_COUNTS_PER_DEGREE 4  //todo: measure real number
...
StepperAxis::StepperAxis(int16_t default_position, // default position
  ...
  pinMode(this->potentiometer_pin, INPUT);
  this->read_position();
...
int16_t StepperAxis::read_position(){
  int16_t raw_counts = analogRead(this->potentiometer_pin);
  status.raw_counts = raw_counts;
  this->current_position = (raw_counts - this->zero_counts) * 10 / STEPPER_COUNTS_PER_DEGREE;
