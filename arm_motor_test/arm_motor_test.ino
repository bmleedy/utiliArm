...
// todo: MECHANICAL  add zero hashmarks and calibration
//                   indicators on all joints and adaptors
// todo: MECHANICAL  add holes for flange (in addition to
//                   existing holes)
// todo: MECHANICAL  add slots/holes for installing the
//                   flange
// todo: ELECTRICAL  add buttons (rocker switch) to
//                   manually move the arm axes
// todo: ELECTRICAL  add a killswitch to stop motion
//                   on the big axes
...
#define WRIST_BEND_AXIS_COUNTS_AT_CENTER   90
...
#define WRIST_ROT_AXIS_COUNTS_AT_CENTER   90
// center position for the servo
...
#define CLAW_AXIS_COUNTS_AT_CENTER   90
// center position for the servo
...
// Class to read poses off of EEPROM storage:
// todo: make shared instead of copied over
...
// Holds the most recent command from serial protocol (if any)
...
void setup() {
  ...
  Serial.print(F("Routine to follow: ["));
  Serial.print(storage->get_num_records());
  Serial.println("]");
  int i = 0;
  pose read_position;
  ...
  base = reinterpret_cast<ArmAxis *>(new StepperAxis(BASE_DEFAULT_POSITION,
       ...
       BASE_COUNTS_AT_CENTER,  // raw feedback position at center (zero)
       BASE_AXIS_MIN, false));  // min position, tenths of degrees from center

  Serial.println(F("Initializing Axis 0.5...(shoulder)"));

  // move shoulder to zero and check for success;
  shoulder = reinterpret_cast<ArmAxis *>(new GearmotorAxis(0,  // tenths deg
       ...
       SHOULDER_COUNTS_AT_CENTER,  // raw feedback position at center(zero)
       SHOULDER_AXIS_MIN));  // min position, tenths of degrees from center
  Serial.println(F("Initializing Axis 1...(elbow)"));

  // move shoulder to zero and check for success;
  elbow = reinterpret_cast<ArmAxis *>(new GearmotorAxis(   0,  // tenths deg
  ...
  wrist_bend = reinterpret_cast<ArmAxis *>(new ServoAxis(0,  // default position
       WRIST_BEND_AXIS_PIN,                  // output pin
       WRIST_BEND_AXIS_MAX,                  // max position (tenths)
       WRIST_BEND_AXIS_COUNTS_AT_CENTER,     // what servo angle is zero
       WRIST_BEND_AXIS_MIN));                // min position (tenths)
  Serial.println(F("Initializing Axis 3 (wrist rotation)..."));
  wrist_rot = reinterpret_cast<ArmAxis *>(new ServoAxis(0,  // default position
       WRIST_ROT_AXIS_PIN,                  // output pin
       WRIST_ROT_AXIS_MAX,                  // max position (tenths)
       WRIST_ROT_AXIS_COUNTS_AT_CENTER,     // what servo angle is zero
       WRIST_ROT_AXIS_MIN));
  Serial.println(F("Initializing Axis 4 (claw)..."));
  claw = reinterpret_cast<ArmAxis *>(new ServoAxis(0,     // default position
       CLAW_AXIS_PIN,                  // output pin
       CLAW_AXIS_MAX,                  // max position (tenths)
       CLAW_AXIS_COUNTS_AT_CENTER,
       CLAW_AXIS_MIN));
...
uint32_t last_print_micros = 0;
uint32_t last_print_iter = 0;
uint32_t auto_timeout = 0;
uint32_t loop_iterator = 0;
...
void loop() {
  ...
  if (serial_protocol_data.get_last_serial() == 0) {  // no commands received
    ...
    if (millis() >= auto_timeout) {  // used up time, move to next pose
      ...
      Serial.print(F("setting desired to: "));
      storage->dump_pose(&read_position, storage->get_current_record());
      ...
      auto_timeout = millis() + read_position.duration_ms;
      // note, ignoring overflow here :-S
    }
  } else {
    // fill commands with last protocol message received
    if (serial_protocol_data.get_last_serial() != last_command_serial) {
      shoulder->set_desired_position(
        serial_protocol_data.get_axis(0).position * 10);
      elbow->set_desired_position(
        serial_protocol_data.get_axis(1).position * 10);
      wrist_bend->set_desired_position(
        serial_protocol_data.get_axis(2).position * 10);
      wrist_rot->set_desired_position(
        serial_protocol_data.get_axis(3).position * 10);
      claw->set_desired_position(
        serial_protocol_data.get_axis(4).position * 10);
      base->set_desired_position(
        serial_protocol_data.get_axis(5).position * 10);
      last_command_serial = serial_protocol_data.get_last_serial();
      Serial.print(F("Following command number "));
      Serial.println(last_command_serial);
    }
  }
  ...
  if ((millis() - last_print_ms) >= PRINT_EVERY_MS) {
    ...
    Serial.print((micros()-last_print_micros) / (loop_iterator));
    // todo: ignores micros overflow
    // Shoulder position
