...
void run_base_calibration() {
  ...
  printok();
  Serial.println(F("Now, press and release the limit switch to "
                   "step forward to the calibration line."));
  ...
  printok();
  Serial.print(F("The calibration point position is "));
  Serial.print(position_counts); Serial.println(F(" counts"));
  Serial.println(F("\n\n exiting calibration\n"));
  printhelp();
}

void run_gearmotor_calibration(uint8_t sensor_pin,
  ...
  Serial.println(F("Press and hold the base limit switch to "
                   "move the shoulder in the POSITIVE direction."));
  while (digitalRead(BASE_SENSOR) == HIGH) {
    delay(10);
  }
  ...
  printok();
    Serial.println(F("Press and hold the base limit switch to move "
                     "the shoulder in the NEGATIVE direction."));
  while (digitalRead(BASE_SENSOR) == HIGH) {
    delay(10);
  }
...
void run_shoulder_calibration() {
  run_gearmotor_calibration(SHOULDER_AXIS,
                            SHOULDER_POSITIVE_PIN,
                            SHOULDER_NEGATIVE_PIN);
}
...
void calibrate_servo(Servo servo) {
  ...
  Serial.println(F("Press and hold the base limit switch to "
                   "move the shoulder in the NEGATIVE direction."));
  while (digitalRead(BASE_SENSOR) == LOW) {
    if (servo_position+1 <= 180)
      servo.write(++servo_position);
    delay(500);
...
void setup() {
  ...
  pinMode(SHOULDER_POSITIVE_PIN, OUTPUT),
  digitalWrite(SHOULDER_POSITIVE_PIN, LOW);
  pinMode(SHOULDER_NEGATIVE_PIN, OUTPUT),
  digitalWrite(SHOULDER_NEGATIVE_PIN, LOW);
  pinMode(SHOULDER_ENA_PIN, OUTPUT),
  digitalWrite(SHOULDER_ENA_PIN, HIGH);
  ...
  wrist_bend_servo.attach(WRIST_BEND_AXIS_PIN);
  wrist_bend_servo.write(SERVO_DEFAULT_ANGLE);

  Serial.println(F("Initializing Wrist Rotation..."));
  wrist_rot_servo.attach(WRIST_ROT_AXIS_PIN);
  wrist_rot_servo.write(SERVO_DEFAULT_ANGLE);
