...
extern "C" void move(void * pvParams){
  ...
  stepTaskParameters * params = (stepTaskParameters *)pvParams;
  ...
  ESP_LOGI(STEPPERAXIS_TAG,
	   "move(): Move command received. Desired: %d millideg Actual: %d Delta: %d",
	   params->destination, *params->position, motion_needed);
  ...
  if(params->speed == 0 || params->steps_per_rev == 0)
    ESP_LOGE(STEPPERAXIS_TAG,
	     "move(): invalid speed or steps per rev: %d, %d",
...
StepperAxis::StepperAxis(uint8_t max_degrees,
			 uint8_t min_degrees,
...
bool StepperAxis::go_to(uint8_t position_deg) {
  ESP_LOGD(STEPPERAXIS_TAG,
	   "go_to(): Defaulting speed to %d centidegrees/s",
...
bool StepperAxis::go_to(uint8_t position_deg, uint16_t speed) {
  ESP_LOGD(STEPPERAXIS_TAG, "go_to(%d,%d): Command received",
	   position_deg, speed);
  ...
  if(!ready_){
    ESP_LOGE(STEPPERAXIS_TAG, "Stepper is not ready to move. Ignoring command.");
    return false;
  }
  ...
  if(blocking_) {
    ...
    move( (void*) &task_params_ );
  }
  else {
    xTaskCreate(&move, "move_stepper", 2000, (void *) &task_params_, 5, NULL);
  }
...
bool StepperAxis::halt() {
  if(stepper_task_ == NULL){
    ESP_LOGD(STEPPERAXIS_TAG,
	     "halt(): No motion task to delete");
  } else {
    ESP_LOGD(STEPPERAXIS_TAG,
	     "halt(): Stopping motion task: %d", (int)stepper_task_);
