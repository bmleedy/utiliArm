...
bool SharedKeyStore::insert_or_update(std::string key, std::string value) {
  ...
  if (has_key(key)) {
    ...
    if (xSemaphoreTake(write_mutex_, 10)) {   // take semaphore
      ...
      vTaskSuspendAll();  //do not switch off of me (does not suspend interrupts)
    ...
    if (xSemaphoreTake(write_mutex_, 10)) {   // take semaphore
      ...
      ESP_LOGE(KEYSTORE_TAG, "failed to add key %s (could not take semaphore)",key.c_str());
      rv = false;  // failed to write
    }

  } else {
    //not enough room, print an error and return false
    ESP_LOGE(KEYSTORE_TAG, "failed to add key %s because keystore is full",key.c_str());
  }
...
bool SharedKeyStore::get(std::string key, std::string * value) {
  if (has_key(key)) {
    ...
    ESP_LOGE(KEYSTORE_TAG, "failed to get key [%s]. Key not in keystore.", key.c_str());
  }
    return true;
}
