...
esp_err_t axis_post_handler(httpd_req_t *req)
{
  ...
  RobotAxis ** axes = (RobotAxis **)req->user_ctx;
  ...
  request_data_string.assign(content,recv_size); //copy content to local string
...
*/
    request_data_map[request_data_string.substr(start_position,(equal_index-start_position))] =
      request_data_string.substr(equal_index+1,(and_index-equal_index-1));
    
    ESP_LOGD(WEBSERVER_LOG_TAG, "received key-value pair [%d]  %s : %s",
	     start_position,
	     request_data_string.substr(start_position,(equal_index-start_position)).c_str(),
	     request_data_map[request_data_string.substr(start_position,(equal_index-start_position))].c_str());
...
httpd_handle_t start_webserver(RobotAxis ** axes, int32_t num_axes)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
      ...
      uri_get.user_ctx = (void *) axes;
      httpd_register_uri_handler(server, &uri_get);
      uri_post.user_ctx = (void *) axes;
